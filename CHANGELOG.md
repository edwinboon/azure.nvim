...
- Removed default keymaps — no bindings are set unless explicitly configured via `keymaps` in `setup()`. Use `:AzFetchAppSettings` and `:AzPushAppSettings` directly, or set your own bindings to avoid conflicts with other plugins.
